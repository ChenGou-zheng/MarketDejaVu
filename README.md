...
4 个 Excel (60 工作表)          convert_xlsx_to_csv         39 个 CSV (逐笔交易)
     ↓                                                         
39 个 CSV                       rebuild_mtm_nav.py          37 个 MTM 净值文件
+ rqdata 3009 只个股收盘价       (rqdata 盯市 + 质量过滤)    (11 个可信 + 26 个标记)
     ↓                                                         
...
**溢出原因**: KNN 选中了 `交易记录动量趋势`(年化波动 522%)、`交易记录形态`(年化波动 1367%)等含 MTM 重建噪声的策略。这些策略的单日收益率可达 +50%~+1000%(因出入金缺失导致的 NAV 跳变)。持有一个 N=20 日块后, 复利计算使 float64 溢出(>1e308)。
...
| **纯 MTM(无过滤, 溢出)** | **37** | **全部 MTM 不过滤** | **3** | **∞%(溢出)** | **∞** | **91.4%** | 91.99% | **∞%** | **90.3%** |
| **纯 MTM(无过滤, 裁剪±20%)** | **37** | **同上+日收益裁剪** | **3** | **-37.56%** | **-0.480** | **44.4%** | 98.02% | **+22.10%** | 54.8% |
| **v4.0 全池** | **63** | **39 映射+13 ETF+11 MTM** | **3** | **+23.66%** | **0.311** | 50.4% | 31.43% | **+6.77%** | **54.8%** |
