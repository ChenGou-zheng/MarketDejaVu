"""
Ablation + robustness — MTM-focused version.

4 pools based on MTM strategies:
  1. MTM-unfiltered:  all available MTM (31, no quality filter)
  2. MTM-filtered:    only quality-passing MTM (10)
  3. MTM-unfiltered+ETF: 31 MTM + 3 ETF
  4. MTM-filtered+ETF:   10 MTM + 3 ETF

4 time windows: T0(2020+), T1(2024+), T2(2025+), T3(2026+)
"""
...
def run_similarity(features: pd.DataFrame, labels: pd.DataFrame) -> pd.DataFrame:
    ...
    for i, t in enumerate(features.index):
        ...
        for s_idx, s_name in enumerate(strategy_names):
            ...
            mu_excess = np.average(x_v, weights=w_v)
            strategy_results.append({
                "strategy": s_name, "n_samples": int(valid.sum()),
                "mean_excess": mu_excess, "t_stat": t_stat, "p_value": p_val,
            })
...
def run_backtest(decisions: pd.DataFrame, returns: pd.DataFrame,
    ...
    sig = decisions[decisions["is_significant"]].copy()
    strategies = [c for c in returns.columns]
    all_dates = returns.index.intersection(hs300_ret.index)
    all_dates = all_dates[all_dates >= sig["decision_date"].min()]
    ...
    for date in all_dates:
        if days_since == 0:
            recent = sig[sig["decision_date"] <= date]
            if recent.empty:
                current_strategies = []
            else:
                ...
                if eligible.empty:
                    eligible = candidates
                current_strategies = eligible.sort_values("mean_excess", ascending=False)["strategy"].head(TOP_K).tolist()
        ...
        portfolio_nav.append(portfolio_nav[-1] * (1 + daily_ret))
        daily_records.append({
            "trade_date": date, "excess_return": excess,
            "portfolio_return": daily_ret, "benchmark_return": hs300_ret.loc[date],
        })
...
def main():
    print("=" * 70)
    print("  Ablation Experiment — MTM Focus")
    print("=" * 70)
    ...
    print(f"\nMTM all: {len(mtm_all)}  MTM good: {len(mtm_good)}  ETF: {len(etf_all)}")
    ...
    # Build full labels once (all MTM + ETF)
    all_keys = mtm_all + etf_all
    print(f"\nBuilding labels for {len(all_keys)} strategies...")
    ...
    for pool_name, pool_keys in pools.items():
        ...
        decisions = run_similarity(pf, pl)
        if decisions.empty:
            continue
        ...
        bt.to_csv(TABLES_DIR / f"ablation_{pool_name.replace('+','_')}.csv", index=False)

    # Print table
    print("\n\n" + "=" * 90)
    print("  FINAL: Ablation × Time Period")
    print("=" * 90)
    ...
    pd.DataFrame(rows).to_csv(TABLES_DIR / "ablation_results.csv", index=False, encoding="utf-8-sig")
    print(f"\n[DONE] Saved to output/tables/ablation_results.csv")
