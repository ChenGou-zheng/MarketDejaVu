"""
Ablation v2: KNN + Rank (no FDR) + N=10.

Changes from v1:
  - No FDR significance test: all strategies are "significant", backtest picks Top-3 by mean_excess
  - N_DAYS reduced from 20 to 10
  - All 4 pools × 4 time periods

Results compared to v1 (FDR + N=20) in report.md.
"""
...
def passes_qf(key: str) -> bool:
    ...
    return (ret.abs() > 0.2).mean() <= 0.03 and ret.std() * np.sqrt(252) <= 2.0
...
def run_similarity(features: pd.DataFrame, labels: pd.DataFrame) -> pd.DataFrame:
    """KNN + rank by mean_excess (no FDR). Every strategy is 'significant'."""
    ...
    for i, t in enumerate(features.index):
        ...
        for s_idx, s_name in enumerate(strategy_names):
            ...
            strategy_results.append({
                ...
                "is_significant": True,  # ← no FDR, all strategies are candidates
                "decision_date": t,
            })
...
def run_backtest(decisions: pd.DataFrame, returns: pd.DataFrame,
    ...
    """Top-K backtest with N_DAYS holding period."""
...
def main():
    print("=" * 70)
    print("  Ablation v2: KNN+Rank (no FDR) + N=10")
    print("=" * 70)
    ...
    for pool_name, pool_keys in pools.items():
        ...
        bt.to_csv(TABLES_DIR / f"ablation_v2_{pool_name.replace('+','_')}.csv", index=False)

    # Print table
    print("\n\n" + "=" * 90)
    print("  KNN+Rank(non-FDR) + N=10  |  Ablation × Time Period")
    print("=" * 90)
    ...
    pd.DataFrame(rows).to_csv(TABLES_DIR / "ablation_v2_results.csv", index=False, encoding="utf-8-sig")
    print(f"\n[DONE] Saved to output/tables/ablation_v2_results.csv")
