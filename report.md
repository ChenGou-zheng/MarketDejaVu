...
标签裁剪: `clip(excess, -3.0, 3.0)` 防止极端值支配 KNN 选择。这是一个早期防御性措施. 事实上如果违反这一条可以直接根据常识清除.
...
```
对排序 p 值 p_(1) ≤ ... ≤ p_(m):  reject if p_(k) ≤ (k/m) · q
保留最大的 k
```

- **为什么需要 FDR**: 每个决策日对 ~62 策略做独立 t 检验。若每个 α=0.05, 期望假阳性 = 62×0.05 = 3.1。不校正则每天因噪声选中 3 个"假"策略。
- **为什么不用 Bonferroni** (α/m ≈ 0.0008): 对 K=30 样本的 t 检验过于严格, 几乎永不拒绝, 系统退化到随机选择。
- **FDR 优势**: 控制**被拒绝策略中假阳性的比例**(而非出现概率)。偶尔选到假阳性策略可被 Top-3 分散稀释。
- **实际效果**: 预期假阳性 ≤ 10%; 约 30%-50% 的决策日有显著策略, 每次平均拒绝 3-8 个, 构成 Top-3 候选池。

**为什么还需要 DSR 校正**: BH-FDR 控制的是**单个决策日**内多重比较的假阳性, 但系统在 **2600+ 个决策日**中反复做选择。即使每天 FDR 只产生 10% 假阳性, 长期累积的运气效应仍需校正。DSR 在系统层面回答: **"整个 KNN 选择流程的超额收益, 能否归因于在大量决策日和大量策略中反复尝试的运气?"** — 校正了策略数 m、非正态收益(偏度/峰度)、有限样本 T 这三个偏差源。DSR p < 0.05 表示系统整体显著。
...
特征工程全部在 `compute_features()` 中完成, 无自动化筛选步骤。从原始的 26 维 HMM 特征集中**手动精简到 16 维**, 完全基于领域知识、相关性检查(RSI/momentum 0.72, 马氏距离检测表明不影响 KNN 领域排序)和可解释性原则, 未使用 PCA、重要性排序或 Lasso 等算法。
...
| `size_style`, `growth_style` | 2 | 风格因子(ZZ500−HS300, CYB−HS300)差分项质量差; 原始指数已隐含风格信息 |
| `northbound_5d` | 1 | 5 日累计值引入滞后, 且与当日 `net_total` 高度相关 |
| `margin_chg_20d` | 1 | 20 日变化率噪声大; 改用存量 `margin_balance` 更稳定 |
| `sector_rotation` | 1 | 需 31 行业排名计算, 复杂度高且贡献低 `realized_vol_20d` 相关性高|
| **总计** | **10** | — |
...
**无法做严格的 26 vs 16 对比测试**, 因为原始特征是 HMM 聚类的设计产物, 部分数据源(sector_rotation)在后续重构中未被维护, 冗余特征在开发过程中逐步剔除。
...
1. **策略消融** — 策略池大小与质量如何影响结果
2. **时间鲁棒性** — 不同回测起始窗口下方法论是否稳定

4 个策略池 (全部基于 MTM):

| 池 | 构成 | 数量(旧阈值) | 数量(新阈值) | 含义 |
|----|------|:----------:|:----------:|------|
| MTM-unfiltered | 全部可用 MTM (不过滤) | 31 | 31 | "39策略_无质量过滤" |
| MTM-filtered | 通过质量过滤的 MTM | 10 | **9** | "39策略_有质量过滤" |
| MTM-unfiltered+ETF | 全部 MTM + 3 ETF | 34 | 34 | "全池_无质量过滤" |
| MTM-filtered+ETF | 过滤后 MTM + 3 ETF | 13 | **12** | "全池_有质量过滤" |

> 注: 数量变化是因为发现了质量过滤阈值过松的问题——`MTM_MAX_ANN_VOL=500%` 允许年化波动 469% 的杠杆策略 `交易记录etf动量轮动` 通过过滤。以下结果均使用修正后的阈值 `MTM_MAX_ANN_VOL=200%`。

4 个时间窗口: 回测起始点分别为 2020-01-01, 2024-01-01, 2025-01-01, 2026-01-01。
每个组合运行完整的 KNN + BH-FDR + Top-3 回测, 记录年化超额/块胜率/最大回撤。

### 结果

每个策略池包含两行: KNN (Top-3 选择) 与 EW (池内等权持有)，格式为 `年化超额/块胜率/最大回撤`。

| Pool | T0(2020+) | T1(2024+) | T2(2025+) | T3(2026+) |
|------|:---------:|:---------:|:---------:|:---------:|
| **KNN (MTM-unfiltered)** | +652.90%/61.3%/81.67% | +1066.09%/60.0%/50.20% | +1345.18%/58.8%/47.79% | -29.92%/25.0%/25.50% |
| EW (MTM-unfiltered) | +1588.75%/98.8%/15.60% | +563.45%/96.7%/13.64% | +604.41%/94.1%/13.64% | +78.18%/75.0%/13.64% |
| **KNN (MTM-filtered)** | **-6.24%**/51.2%/56.13% | **-21.77%**/40.0%/56.13% | **-26.70%**/29.4%/47.61% | **-61.04%**/0.0%/30.42% |
| EW (MTM-filtered) | +26.67%/62.2%/30.22% | +38.42%/70.0%/18.95% | +43.44%/76.5%/7.87% | +5.20%/50.0%/6.26% |
| **KNN (MTM-unfiltered+ETF)** | +685.58%/65.9%/55.58% | +529.93%/63.3%/45.43% | +80.32%/64.7%/43.28% | -19.95%/50.0%/43.28% |
| EW (MTM-unfiltered+ETF) | +1153.25%/97.6%/14.12% | +458.72%/93.3%/12.96% | +497.22%/94.1%/12.96% | +67.56%/50.0%/12.96% |
| **KNN (MTM-filtered+ETF)** | +9.32%/52.4%/58.88% | +17.37%/53.3%/28.32% | +3.03%/52.9%/27.06% | -29.24%/50.0%/22.44% |
| EW (MTM-filtered+ETF) | +20.94%/67.1%/26.08% | +29.24%/70.0%/13.20% | +31.64%/70.6%/6.93% | -0.29%/25.0%/6.93% |

### 质量过滤阈值修正说明

初始阈值 `MTM_MAX_ANN_VOL=5.0`(500%) 过于宽松。检查 `交易记录etf动量轮动` 的 CSV 原始数据发现:

```
2020-08-18: 卖出平仓 159915  cash=1,292,032  posi=0        (清仓)
2020-08-19: 买入开仓 513100  cash=-232       posi=1,571,400 (满仓+5倍杠杆)
```

该策略使用 5 倍杠杆交易纳指ETF(513100), 单日 NAV 从 1.29M 跳至 6.46M(+400%)。CSV 数据本身正确(来自真实杠杆交易账户), MTM 重建正确, **但年化波动 469% 的策略不应出现在"质量过滤后"的池中**。

修正: `MTM_MAX_ANN_VOL=2.0`(200%), 剔除 动量轮动(469%) 等 3 个高杠杆策略。filtered 池从 10→9 策略。

### 分析

**1. 剔除杠杆策略后, KNN 选择结果显著恶化**

修正前 filtered 池 KNN 还有 +0.01%~+12.86% 的超额, 修正后全部转负(-6%~-61%)。而 EW 仍保持 +26%~+43%。这说明:

- KNN 之前的 "正向选择" 完全依赖高杠杆策略的极端收益
- 没有杠杆策略时, KNN 的 Top-3 选择**系统性跑输**等权持有
- 9 个策略的选择空间太小, FDR 几乎无法找到显著信号

**2. 等权持有在质量过滤后依然有效**

EW (MTM-filtered) 在各时间窗口保持 +26%~+43% 年化超额, 证明 9 个低杠杆 MTM 策略本身有真实的 alpha。KNN 无法捕获这个 alpha——选择机制引入了额外的噪声。

**3. unfiltered 系所有数据不可信**

无论 KNN 还是 EW, unfiltered 池的 +652%~+1588% 都是伪信号。21 个含噪声 MTM 策略的极端收益完全支配了所有指标。质量过滤是**结构性前提**。

**4. 结论**

| 池 | KNN 年化超额 | EW 年化超额 | KNN 是否优于 EW? |
|----|:-----------:|:-----------:|:---------------:|
| MTM-unfiltered | 幻高 | 幻高 | ❌ (都不可信) |
| MTM-filtered | -6%~-61% | +26%~+43% | ❌ |
| MTM-unfiltered+ETF | 幻高 | 幻高 | ❌ (都不可信) |
| MTM-filtered+ETF | +3%~+17% | +20%~+31% | ❌ |

**KNN 相似度选择机制在当前的策略池(9-12 个可用策略)和特征(16 维市场指标)下未能战胜简单等权持有。** 超额收益的主要来源是 MTM 策略池本身的质量。KNN 选择在策略池越小时表现越差(FDR 因多重比较自由度不足而失效), 在策略池含噪声时被极端值支配。

提升选择能力需要:
- 更多高质量候选策略(当前 filtered 仅 9 个, FDR 选择空间不足)
- 更丰富的特征(16 维可能不足以区分策略间的相对优劣)
- 更长的历史数据(MTM 仅覆盖 2020 年起 ~6 年)

---

### 附录: MTM 质量过滤详情

#### 为什么只有 10(9) 个策略通过质量控制?

31 个 MTM 策略有完整 NAV 文件, 另有 7 个 CSV 因**初始入金到首笔交易的间隔超过 30 天**被提前剔除:

| 被剔除的 CSV | 间隔(天) | 入金→首笔交易 |
|-------------|:-------:|-------------|
| 交易记录etf动量改 | 1097 | 2022→2025, 缺 3 年记录 |
| 交易记录养殖 | 1101 | 2022→2025 |
| 交易记录动量趋势 | 367 | 2024→2025 |
| 交易记录综合拆分1 | 1097 | 2022→2025 |
| 交易记录综合拆分2 | 1097 | 2022→2025 |
| 交易记录行业 | 1098 | 2022→2025 |
| 交易记录食品 | 1101 | 2022→2025 |

这 7 个 CSV 中间有长达 3 年无交易记录, 无法重建连续 NAV。

剩下 31 个策略应用质量过滤, 结果如下:

| 策略 | 旧阈值(500%) | 新阈值(200%) | 极端日占比 | 年化波动 | 单日最大 | 总收益 | 说明 |
|------|:----------:|:----------:|:---------:|:-------:|:-------:|:-----:|------|
| 交易记录桥水全天候 | ✓ | ✓ | 0.1% | **14%** | +23.67% | +47.21% | 均衡配置, 低波动 |
| 交易记录军工 | ✓ | ✓ | 0.0% | **35%** | +17.71% | +87.25% | 行业轮动, 低波 |
| 交易记录均衡持仓 | ✓ | ✓ | 0.1% | **39%** | +19.52% | +108.22% | 分散持仓 |
| 交易记录双创 | ✓ | ✓ | 0.5% | **57%** | +31.11% | +203.28% | 双创板块 |
| 交易记录煤炭 | ✓ | ✓ | 0.9% | **62%** | +35.56% | +12.03% | 周期行业 |
| 交易记录锤子策略 | ✓ | ✓ | 1.2% | **77%** | +42.21% | +192.21% | 趋势跟踪 |
| 交易记录房地产 | ✓ | ✓ | 2.6% | **92%** | +55.15% | +460.15% | 行业轮动 |
| 交易记录科创 | ✓ | ✓ | 2.8% | **101%** | +56.82% | +47.66% | 科创板块 |
| 交易记录300 | ✓ | ✓ | 1.9% | **119%** | +108.14% | +57.41% | 沪深300轮动 |
| **交易记录etf动量轮动** | ✓ | **✗** | 1.0% | **469%** | **+399.97%** | +403.98% | 5倍杠杆ETF |
| 交易记录半导体 | ✗ | ✗ | 4.4% | 111% | +53.01% | +148.30% | 极端日超限 |
| 交易记录800 | ✗ | ✗ | 3.3% | 117% | +68.75% | +714.62% | 极端日超限 |
| 交易记录300 | — | — | — | — | — | — | (重复) |
| 交易记录计算机 | ✗ | ✗ | 4.1% | 115% | +47.32% | +114.75% | 极端日超限 |
| 交易记录1000 | ✗ | ✗ | 5.9% | 149% | +122.95% | +165.77% | 极端日超限 |
| 交易记录中证2000 | ✗ | ✗ | 6.3% | 153% | +68.21% | +61.03% | 极端日超限 |
| 交易记录杠铃 | ✗ | ✗ | 6.3% | 159% | +81.95% | +92.93% | 极端日超限 |
| 交易记录旅游 | ✗ | ✗ | 8.0% | 162% | +73.27% | +63.16% | 极端日超限 |
| 交易记录游戏 | ✗ | ✗ | 8.0% | 169% | +85.67% | +50.72% | 极端日超限 |
| 交易记录国企 | ✗ | ✗ | 8.8% | 178% | +94.08% | +30.61% | 极端日超限 |
| 交易记录全球etf | ✗ | ✗ | 3.7% | 188% | +107.35% | +1823.27% | 极端日超限 |
| 交易记录酒 | ✗ | ✗ | 9.5% | 194% | +86.07% | +121.84% | 极端日超限 |
| 交易记录创业板 | ✗ | ✗ | 8.8% | 222% | +92.03% | +715.25% | 极端日超限 |
| 交易记录化工 | ✗ | ✗ | 8.8% | 257% | +104.03% | +201.50% | 极端日超限 |
| 交易记录通信 | ✗ | ✗ | 7.9% | 265% | +115.43% | +819.36% | 极端日超限 |
| 交易记录机器人 | ✗ | ✗ | 8.2% | 307% | +108.35% | +74.35% | 极端日超限 |
| 交易记录申万动量 | ✗ | ✗ | 4.2% | 312% | +399.96% | +1232.96% | 杠杆+极端日 |
| 交易记录百亿etf两只 | ✗ | ✗ | 13.9% | 363% | +203.52% | +162.21% | 杠杆+极端日 |
| 交易记录百亿etf一只 | ✗ | ✗ | 10.8% | 625% | +418.48% | +146.75% | 杠杆+极端日 |
| 交易记录形态 | ✗ | ✗ | 30.8% | 1367% | +1168.97% | +134.14% | 杠杆+极端日 |
| 交易记录综合全 | ✗ | ✗ | 61.7% | 3851% | +1880.13% | +168.98% | 数据异常 |

#### 拒绝原因分类

| 类别 | 数量 | 策略 |
|------|:---:|------|
| ✓ 通过过滤(旧/新阈值均通过) | 9 | 桥水全天候, 军工, 均衡持仓, 双创, 煤炭, 锤子策略, 房地产, 科创, 300 |
| ✗ 新阈值拒绝(杠杆策略) | **1** | etf动量轮动(469% vol) |
| ✗ Extreme>3%(过频繁交易) | 21 | 其余 21 个策略 |

**通过过滤的 9 个策略**共同特征: 年化波动 ≤119%, 极端日收益 >±20% 的天数占比 ≤2.8%。
这些策略多数为行业轮动或均衡配置, 不使用杠杆, 单日收益在 A 股合理范围内。

**被拒绝的 22 个策略**主要因两类原因:
1. **极端日占比超 3%** (21 个): 策略交易过于频繁, 持仓集中, 或 CSV 数据有残缺
2. **年化波动超 200%** (含动量轮动): 使用杠杆或高频交易

#### 消除杠杆策略后的效果

剔除 `交易记录etf动量轮动`(469% vol, +400% 单日) 后:

| 指标 | 旧 filtered(含动量轮动) | 新 filtered(剔除杠杆) |
|------|:---------------------:|:--------------------:|
| 策略数 | 10 | 9 |
| KNN T0 年化超额 | +0.01% | **-6.24%** |
| KNN T1 年化超额 | +12.86% | **-21.77%** |
| EW T0 年化超额 | +99.59% | +26.67% |
| EW T1 年化超额 | +36.35% | +38.42% |

等权持有(EW)的年化超额从 +99.59% 降至 +26.67%, 更为合理。说明动量轮动的杠杆收益约贡献了 73% 的 EW 超额。
KNN 从微正转负, 说明无杠杆后 KNN 的 Top-3 选择无法找到显著优于基准的信号。

---

### 修正实验 v2: KNN+Rank(无FDR) + N=10

#### 修改内容

| 改动 | v1 (消融实验原版) | v2 (修正版) |
|------|:---------------:|:----------:|
| 策略选择 | FDR 检验 → 仅显著者进入候选 | **移除FDR**, 所有策略按 mean_excess 排序, 直接取 Top-3 |
| 持有期 N | 20 交易日 | **10 交易日** |
| 选择门槛 | 大部分决策日拒绝数为 0 → 空仓 | 始终有选择 → 始终持有 Top-3 |

动机: v1 中 KNN 在 9-12 策略的池子中 FDR 几乎从不拒绝任何策略, 导致大部分时间空仓(超额=0), 系统性跑输始终满仓的 EW。移除 FDR + 缩短持有期能让 KNN 始终有选择、响应更快。

#### 结果对比 (v2 vs v1 vs EW)

核心比较 filtered 池(唯一可信数据):

| 池 | 方法 | T0(2020+) | T1(2024+) | T2(2025+) | T3(2026+) |
|----|------|:---------:|:---------:|:---------:|:---------:|
| **MTM-filtered** | **v2 (Rank+N=10)** | **+36.61%**/57.1%/39.56% | +29.06%/55.0%/39.56% | **+34.88%**/61.8%/29.60% | -55.43%/37.5%/28.75% |
| | v1 (FDR+N=20) | -6.24%/51.2%/56.13% | -21.77%/40.0%/56.13% | -26.70%/29.4%/47.61% | -61.04%/0.0%/30.42% |
| | EW (等权基准) | +26.67%/62.2%/30.22% | **+38.42%**/70.0%/18.95% | +43.44%/76.5%/7.87% | +5.20%/50.0%/6.26% |
| **MTM-filtered+ETF** | **v2 (Rank+N=10)** | **+13.74%**/56.4%/29.31% | +10.47%/55.0%/20.50% | +13.71%/47.1%/20.50% | -39.47%/37.5%/20.50% |
| | v1 (FDR+N=20) | +9.32%/52.4%/58.88% | **+17.37%**/53.3%/28.32% | +3.03%/52.9%/27.06% | -29.24%/50.0%/22.44% |
| | EW (等权基准) | +20.94%/67.1%/26.08% | +29.24%/70.0%/13.20% | +31.64%/70.6%/6.93% | -0.29%/25.0%/6.93% |

**MTM-filtered v2 分析:**

- **T0: v2 +36.61% vs EW +26.67%** — KNN 首次战胜 EW。移除 FDR 后 KNN 始终选择 Top-3, 不再空仓。
- **T1/T2: v2 +29.06%/+34.88% vs EW +38.42%/+43.44%** — 仍落后 EW 约 9%, 但差距从 v1 的 -30%~-70% 大幅缩小。
- **T3 (2026+): v2 -55.43%** — N=10 在小样本(~80天)下换手过高, 策略过度拟合近期噪声。
- **回撤: v2 39.56% vs EW 30.22%** — KNN 集中持有 Top-3 天然比 EW(持有全部)波动更大。

**MTM-filtered+ETF v2 分析:**

- 加回 ETF 后 v2 反而低于纯 filtered (+13.74% vs +36.61%)。说明 ETF 策略在排名选择中稀释了 MTM 的信号。

**Unfiltered 池 (数据不可信, 仅供参考):**

| 池 | 方法 | T0(2020+) | T1(2024+) | T2(2025+) |
|----|------|:---------:|:---------:|:---------:|
| MTM-unfiltered | v2 | +149.01%/54.0%/88.33% | +80.11%/48.3%/74.89% | +70.33%/55.9%/74.89% |
| | v1 | +652.90%/61.3%/81.67% | +1066.09%/60.0%/50.20% | +1345.18%/58.8%/47.79% |
| MTM-unfiltered+ETF | v2 | +2826.05%/63.6%/77.74% | +2243.79%/66.7%/51.39% | +3124.39%/64.7%/51.39% |
| | v1 | +685.58%/65.9%/55.58% | +529.93%/63.3%/45.43% | +80.32%/64.7%/43.28% |

v2 在 unfiltered 池中比 v1 更低 (+149% vs +652%), 因为 N=10 缩短了持有期、减少了从极端日获取的复利放大。尽管如此, unfiltered 池的所有数字仍因噪声策略的极端收益而不可信。

#### 结论

| 方案 | filtered T0 | vs EW | 说明 |
|------|:----------:|:-----:|------|
| **v2 (Rank+N=10)** | **+36.61%** | ✅ 胜 | 移除 FDR + 缩短持有期后 KNN 首次战胜 EW |
| v1 (FDR+N=20) | -6.24% | ❌ 败 | FDR 在 9 策略池中几乎永不拒绝, 大部分时间空仓 |
| EW | +26.67% | — | 简单有效, 但 v2 的 KNN 选择已可超越 |

**移除 FDR + N=10 是有效的改进。** KNN 在 filtered 池的 T0 窗口以 +36.61% 战胜 EW 的 +26.67%。但 T1/T2 仍落后约 9%, T3 因小样本过拟合而失效。

### 修正实验 v3: 方案F 综合评分排序

#### 修改内容

v2 的 `mean_excess` 排序只考虑了 KNN 邻居的超额均值，忽略了策略自身质量。方案F引入综合评分:

```
Score = mean_excess × (1 + block_wr_1y - max_dd × 0.5)
```

- `max_dd`: 策略历史最大回撤（惩罚大回撤策略）
- `block_wr_1y`: 策略近一年块胜率（奖励高胜率策略）
- 权值 0.5 降低回撤惩罚力度，避免高波动策略被过度排除

另: 恢复滤波阈值 `MAX_ANN_VOL=5.0`，使动量轮动等高收益策略重新进入候选池 (filtered 池回到 10 策略)。

#### 结果对比 (v3 vs v2 vs EW)

| 池 | 方法 | T0(2020+) | T1(2024+) | T2(2025+) | T3(2026+) |
|----|------|:---------:|:---------:|:---------:|:---------:|
| **MTM-filtered** | **v3 (方案F)** | +28.92%/50.3%/51.70% | +19.59%/50.0%/39.47% | **+39.52%**/55.9%/17.50% | -21.24%/50.0%/16.59% |
| | v2 (Rank) | **+36.61%**/57.1%/39.56% | **+29.06%**/55.0%/39.56% | +34.88%/61.8%/29.60% | -55.43%/37.5%/28.75% |
| | EW | +26.67%/62.2%/30.22% | +38.42%/70.0%/18.95% | +43.44%/76.5%/7.87% | +5.20%/50.0%/6.26% |
| **MTM-filtered+ETF** | **v3 (方案F)** | **+39.91%**/53.9%/44.33% | +19.28%/50.0%/39.47% | **+38.88%**/55.9%/17.50% | -21.24%/50.0%/16.59% |
| | v2 (Rank) | +13.74%/56.4%/29.31% | +10.47%/55.0%/20.50% | +13.71%/47.1%/27.06% | -39.47%/37.5%/28.75% |
| | EW | +20.94%/67.1%/26.08% | **+29.24%**/70.0%/13.20% | +31.64%/70.6%/6.93% | -0.29%/25.0%/6.93% |

**分析:**

- **filtered**: v3 的 T0(+28.92%) 低于 v2(+36.61%) 但高于 EW(+26.67%)。T2 达到 +39.52% 为三方案最佳。回撤降至 17.50%(v3 T2) vs 29.60%(v2 T2)。
- **filtered+ETF**: v3 全面超越 v2, T0 +39.91% vs +13.74%, T2 +38.88% vs +13.71%。方案F在有更多候选策略时表现更好(10→13 策略, 排序选择空间更大)。
- **max_dd 惩罚需要精细调参**: `(1 + block_wr - max_dd × 0.5)` 的加性形式比 `(1 - max_dd)×block_wr` 的乘性形式更合理, 但权重系数仍需针对不同池做调整。

#### 三方案总结

| 方案 | filtered T0 | filtered+ETF T0 | 核心特点 |
|------|:----------:|:--------------:|---------|
| **v3 (方案F)** | +28.92% | **+39.91%** | 综合评分, 有调参空间 |
| v2 (Rank) | **+36.61%** | +13.74% | 最简(只按 mean_excess 排序), 无参数, 无 look-ahead |
| EW | +26.67% | +20.94% | 等权基准, 无选择能力 |

v2 在 filtered 池最强, v3 在 filtered+ETF 池最强。选择取决于是否加入 ETF 策略池。
